...
<html>
    ...
    <body class="bg-gray-200">
        <div class="bg-white p-6 rounded-lg shadow-md grid grid-cols-12">
            <h1 class="text-2xl font-medium col-span-11">Sistema de Reservación del Hotel El Cisne: Lista de Reservaciones</h1>
        ...
        <div class="w-full h-full flex items-center justify-center mt-5">
            <div class="px-4 py-2 w-full mx-10 rounded-lg shadow-md flex bg-white">
                <a href = "${pageContext.request.contextPath}/reservation?action=create"
                   class="bg-indigo-500 hover:bg-indigo-700 text-white py-2 h-1/4 px-4 rounded-lg"
                   >Registrar Reservación</a> 
            </div>
        </div>
                ...
                <table class="table-fixed text-center mx-auto divide-y divide-gray-400"
                    ...
                    <thead>
                        <tr>
                            ...
                            <th class="px-9 py-2">Fecha de Entrada</th>
                            <th class="px-9 py-2">Fecha de Salida</th>
                    ...
                    <tbody class=" divide-y divide-gray-300">
                        <c:forEach items="${reservationsList}" var="reservation">
                            <tr>
                                <td class="px-9 py-5">${reservation.reservationId}</td>
                                <td class="px-9 py-5">${reservation.checkIn}</td>
                                <td class="px-9 py-5">${reservation.checkOut}</td>
                                <td class="px-9 py-5">
                                    <a href = "${pageContext.request.contextPath}/reservation?action=show&packId=${pack.packId}"
                                       class="bg-indigo-500 hover:bg-indigo-700 text-white py-1 px-2 mx-2 rounded-lg"
                                       >Ver</a> 
                                    <a href = "${pageContext.request.contextPath}/reservation?action=edit&packId=${pack.packId}"
                                       class="bg-yellow-500 hover:bg-yellow-700 text-white py-1 px-2 mx-2 rounded-lg"
                                       >Editar</a> 
                                    <a href = "${pageContext.request.contextPath}/reservation?action=delete&packId=${pack.packId}"
